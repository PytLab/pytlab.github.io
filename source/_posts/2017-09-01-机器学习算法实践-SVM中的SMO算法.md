...
当$\\alpha\_{1}^{new}$不在边界，即$0 < \\alpha\_{1}^{new} < C$, 根据KKT条件可知相应的数据点为支持向量，满足$y\_{1}(w^{T} + b) = 1$, 两边同时乘上$y\_{1}$得到$\\sum\_{i=1}^{N}\\alpha\_{i}y\_{i}K\_{i, 1} + b = y\_{1}$, 进而得到$b\_{1}^{new}$的值:
$$
...
当两个乘子$\\alpha\_{1}, \\alpha\_{2}$都在边界上，且$L \\ne H$时，$b1, b2$之间的值就是和KKT条件一直的阈值。SMO选择他们的中点作为新的阈值: $b^{new} = \\frac{b\_{1}^{new} + b\_{2}^{new}}{2}$
